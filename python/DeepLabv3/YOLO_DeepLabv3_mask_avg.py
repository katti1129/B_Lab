#YOLOのバウンディングボックス内のマスク画像の割合とYOLOの信頼値の調和平均の全画像平均
...
def process_and_save_results(image_path, output_dir):
    ...
    if len(yolo_results[0].boxes) == 0:
        ...
        #segmentation_output_path = os.path.join(output_dir, f"segmentation_{os.path.basename(image_path)}")
        #segmentation_overlay.save(segmentation_output_path)
        #print(f"Results saved: {segmentation_output_path}")
        #return
    ...
    for box in yolo_results[0].boxes:
        ...
        for class_id in range(1, num_classes):  # 背景以外のクラス
            ...
            if ratio >= 0.1 or ratio < 0.1:
                font = cv2.FONT_HERSHEY_SIMPLEX
                font_scale = 3
                thickness = 10
                text = (f"confidence of YOLO&DeepLabv3:({confidence_ratio:.2f}%)")
                position = (10, 100)
...
if __name__ == "__main__":
    input_dir = "C:/Users/cpsla/PycharmProjects/segmentation/images/tmp"
    output_dir = "C:/Users/cpsla/PycharmProjects/segmentation/runs_21"
